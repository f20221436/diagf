...
if __name__ == "__main__":
    ...
    for k in ['labels_path', 'label_path', 'labels', 'data_path']:
        if k in cfg: # Check if key exists first
            current_path_to_check = cfg[k]
            print(f"DEBUG: Checking for labels file at: {current_path_to_check}") # <-- ADD THIS LINE
    ...
    if labels_file_path:
        ...
        try:
            if labels_file_path.endswith('.csv'):
                ...
                # Set index if case_id exists, crucial for FastTextLab
                ...
                # Load pickle using pf.load
                labels = pf.load(labels_file_path)
            ...
            labels = None # Ensure labels is None if loading fails

    # --- End replacement ---

    # Keep the fallback logic and error check below this
    if labels is None:
        # fallback: attempting demo path used in repository
        print("Could not load labels from config path, trying fallback demo path...")
        # (Keep the existing fallback logic here)
        # ...
        if labels is None: # Check again after fallback
            raise FileNotFoundError("Could not find or load labels/cases. Checked config path and fallback path. Please verify 'labels_path' in your config and ensure the file is readable.")
